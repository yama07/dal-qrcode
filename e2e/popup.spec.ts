...
test.describe('コンテキストメニュー連携', () => {
  test('コンテキストしている文字列がクエリ経由でポップアップに渡される', async ({
    page,
    dalQrcodeExtension,
  }) => {
    const popupPage = new PopupPage(page, dalQrcodeExtension, {
      text: 'innu/イッヌ',
    });

    await popupPage.goto();
    await popupPage.page.waitForLoadState('load');

    await expect(popupPage.getInputTextarea()).toHaveValue('innu/イッヌ');
  });
});
